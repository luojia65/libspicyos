...
pub fn fs_read(fd: usize, buf: &mut [u8]) -> Result<usize> {
    let (ans, err) = syscall(MODULE_FS, FUNCTION_FS_READ, fd, buf.as_ptr() as usize, buf.len());
    if err != 0 {
...
pub fn fs_write(fd: usize, buf: &[u8]) -> Result<usize> {
    let (ans, err) = syscall(MODULE_FS, FUNCTION_FS_WRITE, fd, buf.as_ptr() as usize, buf.len());
    if err != 0 {
        return Err(()) // todo: an enum
    }
    return Ok(ans)
...
fn syscall(module: usize, function: usize, param0: usize, param1: usize, param2: usize) -> (usize, usize) {
    let ans: usize;
    let err: usize;
    unsafe { llvm_asm!(
        "ecall"
